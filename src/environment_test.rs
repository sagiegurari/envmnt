...
fn set_or_remove_none() {
    let output = set_or_remove::<&str, &str>("TEST_SET_OR_REMOVE_NONE", &None);
...
fn set_or_remove_some() {
    let mut output = set_or_remove("TEST_SET_OR_REMOVE_SOME", &Some("OPTIONAL VALUE"));
    ...
    assert!(!output);
    assert!(!exists("TEST_SET_OR_REMOVE_SOME"));
}
