          ...
          E: Display
{
    match var(&key)
    {
        Ok(valstr) => {
            valstr.parse::<T>()
                .map_err(|e| EnvmntError{
                    kind: ErrorKind::InvalidType(
                        format!("Environment variable {} is of incompatible type {}. Failed to parse with: {}",
            ...
            Err(
                EnvmntError{
                    kind: ErrorKind::Missing(
                        format!("Environment variable '{}' is not set",
                            key.as_ref().to_string_lossy()
                        )
                    )
                }
        ...
        Err(VarError::NotUnicode(osstr)) => {
            Err(
                EnvmntError {
                    kind: ErrorKind::InvalidType(format!("Environment variable is not valid unicode: {:#?}", osstr))
                }
            )
        }
    }
